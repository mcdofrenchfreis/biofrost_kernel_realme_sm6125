...
{
    struct oppo_vooc_chip *chip = container_of(work, struct oppo_vooc_chip, vooc_watchdog_work);
...
*/
        ...
        chip->vops->reply_mcu_data(chip, ret_info, oppo_gauge_get_device_type_for_vooc());
...
bool oppo_vooc_get_allow_reading(void)
{
        if (!g_vooc_chip) {
                ...
                if (g_vooc_chip->support_vooc_by_normal_charger_path && g_vooc_chip->fast_chg_type == CHARGER_SUBTYPE_FASTCHG_VOOC) {
			return true;
                } else {
